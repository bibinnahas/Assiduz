...
file_name = 'sampleEmployee.csv'

sampleenvVars.setVariables()
...
class PDF(FPDF):
    ...
    def personal_details(self):
        ...
        pdf.cell(0, 5, str(os.environ['desc5']) + str(0), 1, 1, 'R')
        ...
        pdf.cell(0, 5, str(os.environ['leave1']) + row[7], 1, 0, 'L')
        pdf.cell(0, 5, str(os.environ['leave2']) + row[8], 1, 1, 'R')
        pdf.para()

    # salary info details
    def salary_details(self):
        pdf.center_sub_title(str(os.environ['center']) + ' (' + datetime.datetime.now().strftime("%B") + " " + str(
...
with open(file_name, 'r') as file:
    ...
    for row in reader:
        ...
        pdf.salary_details()
        pdf.conclusion()
        pdf.output((row[0].split(" ")[0]) + '_' + datetime.datetime.now().strftime("%B") + '_' + str(
            datetime.datetime.now().year) + '.pdf', 'F')
